...
// The resolver's catch-and-continue stops at module load: once we have a
// module, a throw from parseBaisFile is a real parse error about the caller's
// file and must propagate verbatim. Folding it into the loop meant every
// malformed issue was reported as "BAIS parser not found".
...
export async function createBaisIssue(opts: {
	...
	// New ids carry the owning project's scope, not a hardcoded prefix.
...
// Mirror of BAML `cyclic_ids` — Kahn's algorithm keeping the leftovers:
// whatever cannot be dropped is in a dependency cycle or downstream of one.
// Same mirror rationale as filterReadyIssues above.
